...
app.get("/api/health", (_req, res) => {
  res.json(
    ok({
      ...
      storage: "mongoose",  // เปลี่ยนจาก "prisma" → "mongoose"
...
app.get(
  ...
  route(async () => {
    ...
    // populate elders, lineConnections, botnoiMappings แบบ manual (แทน Prisma include)
...
// ✅ Automation jobs (in-memory — ไม่ต้องแก้)
app.get(
...
app.get(
  ...
  route(() => ({
    storage: "mongoose",  // เปลี่ยนจาก "prisma"
    queue: {
...
// ✅ LINE Webhook (signature verify — ไม่ต้องแก้ logic)
