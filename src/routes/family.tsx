      ...
      <div className="page-header mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            จัดการการแจ้งเตือนญาติและผู้ดูแลผู้ป่วย
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            <h2 className="text-sm font-bold flex items-center gap-2 mb-4">
              <MessageSquare className="h-4 w-4 text-primary" /> เทมเพลตข้อความ (LINE)
            </h2>
            ...
            <div className="rounded-lg bg-[#00B900]/10 border border-[#00B900]/20 p-4 relative">
              <p className="text-[10px] font-bold text-[#00B900] mb-2 uppercase tracking-wider">
                LINE Preview
              </p>
          ...
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            <h2 className="text-sm font-bold flex items-center gap-2 mb-4">
              <Clock className="h-4 w-4 text-primary" /> ประวัติการส่งล่าสุด
            </h2>
            <div className="space-y-3">
              {notifications.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-4">ไม่มีประวัติการส่ง</p>
              )}
              {notifications.slice(0, 5).map((n) => (
                <div key={n.id} className="border-b last:border-0 pb-3 last:pb-0">
                  ...
                  <p className="text-xs line-clamp-1 border-l-2 pl-2 text-muted-foreground">
                    {n.message}
                  </p>
                </div>
              ))}
              ...
              >
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {p.caregiverPhone} • {p.familyConsentLevel}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="rounded-lg bg-muted/40 p-3 mb-4 text-sm flex gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {p.carePlan} • สิทธิ์: {p.consentStatus}
                    </p>
                  </div>
                </div>
