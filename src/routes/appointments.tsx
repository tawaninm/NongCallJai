  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5 mb-6">
        ...
        <StatCard
          title="ต้องการรถรับส่ง"
          value={appointments.filter((a) => a.needsTransport).length}
          icon={Car}
        />
        <StatCard
          title="ส่ง Reminder แล้ว"
          value={appointments.filter((a) => a.reminderSent).length}
          icon={CheckCircle}
          variant="green"
        />
        <StatCard
          title="Reminder ล้มเหลว"
          value={appointments.filter((a) => a.reminderFailed).length}
          icon={Bell}
          variant="yellow"
        />
      </div>
      <div className="space-y-6">
        <div>
          <h2 className="section-title flex items-center gap-2">
            <CalendarCheck className="h-5 w-5 text-primary" /> นัดที่กำลังจะมาถึง
          </h2>
              ...
              <tbody>
                {upcoming.map((a) => (
                  <tr
                    ...
                    onClick={() =>
                      navigate({ to: "/patients/$patientId", params: { patientId: a.patientId } })
                    }
                  >
                    ...
                    <td className="px-4 py-3">
                      {a.reminderSent ? "✅" : a.reminderFailed ? "❌" : "⏳"}
                    </td>
                    ...
                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex gap-1">
                        <button
                          onClick={() => toast.success(`ส่ง Reminder: ${a.patientName}`)}
                          className="rounded p-1.5 hover:bg-muted"
                        >
                          <Bell className="h-4 w-4" />
                        </button>
                        <button
                          onClick={() => toast.info(`โทร: ${a.patientName}`)}
                          className="rounded p-1.5 hover:bg-muted"
                        >
        ...
        {missed.length > 0 && (
          <div>
            <h2 className="section-title flex items-center gap-2">
              <CalendarX className="h-5 w-5 text-risk-red" /> ขาดนัด
            </h2>
            <div className="rounded-xl border bg-card overflow-hidden">
              <table className="w-full text-sm">
                <thead className="border-b bg-muted/50">
                ...
                <tbody>
                  {missed.map((a) => (
                    <tr key={a.id} className="border-b last:border-0 hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => toast.info(`โทรนัดใหม่: ${a.patientName}`)}
                          className="rounded p-1.5 hover:bg-muted"
                        >
                          <Phone className="h-4 w-4" />
                        </button>
