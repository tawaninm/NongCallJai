...
function CasesPage() {
  ...
  const moveCase = (patientId: string, newStatus: CaseStatus) => {
    mockStore.updatePatientStatus(
      patientId,
      newStatus,
      userName,
      `ย้ายเป็น ${caseStatusLabels[newStatus]}`,
    );
    ...
    <div>
      <div className="page-header">
        <h1 className="page-title flex items-center gap-2">
          <Columns3 className="h-7 w-7 text-primary" /> Case Management
        </h1>
      </div>
            ...
            <div key={col.status} className={`kanban-column border-t-4 ${col.color}`}>
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-xs font-bold uppercase tracking-wider">
                  {caseStatusLabels[col.status]}
                </h3>
                <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-muted text-[10px] font-bold px-1">
                  {cards.length}
                </span>
              </div>
