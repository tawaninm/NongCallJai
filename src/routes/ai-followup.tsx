          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border bg-background py-1.5 pl-8 pr-3 text-sm outline-none focus:ring-2 focus:ring-primary/20"
              />
            </div>
            <button className="rounded-lg border bg-background p-1.5 text-muted-foreground hover:bg-muted">
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded">
                    {p?.carePlanType === "diabetes"
                      ? "เบาหวาน"
                      : p?.carePlanType === "hypertension"
                        ? "ความดัน"
                        : "ทั่วไป"}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    HN: {current.hn} • {patient.carePlan}
                  </p>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    mockStore.sendToNurseQueue(current.id, userName);
                    toast.success("ส่งเข้าคิวพยาบาลแล้ว");
                  }}
                  ...
                  {current.transcript?.split("\n").map((line, i) => {
                    ...
                    return (
                      <div key={i} className={`flex gap-3 ${isAI ? "" : "flex-row-reverse"}`}>
                        <div
                          className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${isAI ? "bg-primary text-white" : "bg-slate-200 text-slate-600"}`}
                        >
                          {isAI ? <Bot className="h-4 w-4" /> : <User className="h-4 w-4" />}
                        </div>
                        <div
                          className={`flex flex-col ${isAI ? "items-start" : "items-end"} max-w-[80%]`}
                        >
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs font-semibold text-muted-foreground">
                              {isAI ? "AI Agent" : current.patientName}
                            </span>
                            <span className="text-[10px] text-muted-foreground">10:0{i} AM</span>
                          </div>
                          <div
                            className={`p-3 rounded-2xl text-sm shadow-sm ${isAI ? "bg-muted/50 rounded-tl-sm" : "bg-primary text-white rounded-tr-sm"} ${hasRedFlag && !isAI ? "border-2 border-risk-red bg-risk-red text-white" : ""}`}
                          >
                            {content}
                          </div>
                        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">
                        ข้อมูลที่สกัดได้ (Extraction)
                      </p>
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span className="text-muted-foreground">อาการ</span>
                          <span className="font-semibold text-risk-red text-right">
                            {current.symptoms.length > 0 ? current.symptoms.join(", ") : "-"}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span
                            className={`font-semibold text-right ${current.riskLevel === "red" ? "text-risk-red" : current.riskLevel === "yellow" ? "text-risk-yellow" : "text-risk-green"}`}
                          >
                    ...
                    <div className="rounded-lg bg-risk-yellow-bg/50 p-3 flex gap-2">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-risk-yellow mb-1">
                          AI Recommendation
                        </p>
