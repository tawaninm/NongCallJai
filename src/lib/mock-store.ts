/**
 * CareGo v0.1.0 — Prototype Mock Store
...
import {
  ...
  aiFollowUps as initialFollowUps,
  type Patient,
  type AIFollowUp,
  type CaseStatus,
  type RiskLevel,
...
function initAlerts() {
  ...
  // No answer patients
  _followUps
    .filter((f) => f.callStatus === "no_answer")
    .forEach((f) => {
      _topNotifications.push({
        id: uid(),
        title: "ติดต่อผู้ป่วยไม่ได้",
  ...
  reds.forEach((p) => {
    _topNotifications.push({
      ...
      description: `${p.name} — ${p.riskReason}`,
      patientId: p.id,
      type: "red_case",
      read: false,
      timestamp: now(),
...
export const mockStore = {
  ...
  getUnreadCount() {
    return (
      _topNotifications.filter((n) => !n.read).length +
      _alerts.filter((a) => !a.acknowledged).length
    );
  },
  ...
  getFollowUp(id: string) {
    return _followUps.find((f) => f.id === id);
  },
  getActionLogForPatient(patientId: string) {
    return _actionLog.filter((a) => a.patientId === patientId);
  },
  getNotificationsForPatient(patientId: string) {
    return _notificationLog.filter((n) => n.patientId === patientId);
  },
  ...
  updateRiskLevel(patientId: string, riskLevel: RiskLevel, by: string) {
    ...
    if (p) {
      ...
      _actionLog.unshift({
        id: uid(),
        patientId,
        action: `เปลี่ยนระดับความเสี่ยงเป็น ${riskLevel}`,
        performedBy: by,
        timestamp: now(),
        note: "",
        type: "case",
  ...
  referDoctor(patientId: string, doctor: string, by: string) {
    ...
    if (p) {
      ...
      _actionLog.unshift({
        id: uid(),
        patientId,
        action: `ส่งต่อแพทย์ ${doctor || p.assignedDoctor}`,
        performedBy: by,
        timestamp: now(),
        note: "",
        type: "medical",
  ...
  notifyFamily(patientId: string, message: string, template: string, by: string) {
    ...
    if (p) {
      ...
      _notificationLog.unshift({
        ...
        patientId,
        patientName: p.name,
        caregiverName: p.primaryCaregiver,
        type: "LINE",
        message,
        template,
        timestamp: now(),
      ...
      _actionLog.unshift({
        id: uid(),
        patientId,
        action: `แจ้งญาติ — ${p.primaryCaregiver}`,
        performedBy: by,
        timestamp: now(),
        note: message,
        type: "family",
  ...
  sendToNurseQueue(followUpId: string, by: string) {
    ...
    if (f) {
      ...
      if (p) {
        p.caseStatus = "nurse_review";
      }
      _actionLog.unshift({
        id: uid(),
        patientId: f.patientId,
        action: "ส่งเข้าคิวพยาบาล",
        performedBy: by,
        timestamp: now(),
        note: "",
        type: "case",
  ...
  acknowledgeAlert(alertId: string, by: string) {
    ...
    if (a) {
      ...
      _actionLog.unshift({
        ...
        action: `รับทราบการแจ้งเตือน — ${a.title}`,
        performedBy: by,
        timestamp: now(),
        note: "",
        type: "system",
  ...
  addPatient(data: Partial<Patient>): string {
    ...
    const newP: Patient = {
      id,
      hn,
      name: data.name || "ผู้ป่วยใหม่",
      age: data.age || 50,
      gender: data.gender || "ไม่ระบุ",
      phone: data.phone || "",
      carePlan: data.carePlan || "",
      carePlanType: data.carePlanType || "hypertension",
      department: data.department || "อายุรกรรม",
      lastContact: "-",
      symptomSummary: "-",
      ...
      redFlagReason: "",
      yellowFlagReason: "",
      assignedNurse: data.assignedNurse || "พว.สมหญิง",
      assignedDoctor: data.assignedDoctor || "นพ.วิชัย",
      caseStatus: "pending",
      consentStatus: data.consentStatus || "ยินยอม",
      ...
      latestBloodSugar: data.latestBloodSugar || "-",
      latestWeightChange: data.latestWeightChange || "-",
      medicationAdherence: "-",
      pharmacistStatus: "ไม่ต้องตรวจ",
      familyConsentLevel: data.familyConsentLevel || "เต็มรูปแบบ",
      medications: [],
    };
    ...
    _actionLog.unshift({
      id: uid(),
      patientId: id,
      action: `ลงทะเบียนผู้ป่วยใหม่ — ${newP.name}`,
      performedBy: "ระบบ",
      timestamp: now(),
      note: "",
      type: "system",
  ...
  createPatientFromRegistration(formData: {
    ...
    return this.addPatient({
      ...
      carePlan: formData.disease,
      carePlanType: formData.carePlanType as CarePlanType,
      assignedDoctor: formData.doctor,
      assignedNurse: formData.nurse,
      primaryCaregiver: formData.caregiverName,
